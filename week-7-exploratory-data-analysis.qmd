...
pollution |> 
  group_by(size) |> #only new line of code
  summarize(mean = mean(amount),
            sum = sum(amount),
            n = n())
```
...
top_10 |> 
  ggplot(mapping = aes(x = fct_reorder(name, 
                                       desc(total)),
                       y = total, 
                       fill = sex)) + 
  geom_col() + 
  scale_fill_brewer() +
  theme_light() +
  labs(x = "Top 10 Popular Names in Top 500 in US",
       y = "Total Number of Infants Born in Top 500",
       fill = "sex",
       title = "Names Since 2005")
```
